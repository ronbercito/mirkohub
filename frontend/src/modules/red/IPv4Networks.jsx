        ...
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm p-4 flex items-center justify-center">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-md w-full p-6 shadow-2xl">
            ...
            <form onSubmit={save} className="pt-4 space-y-4 text-xs">
              <Field label="Nombre de la red"><input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Ej. RED 15" className="input" /></Field>
              <Field label="MikroTik"><select required value={form.router_id} onChange={(e) => setForm({ ...form, router_id: e.target.value })} className="input"><option value="" disabled>Selecciona un MikroTik</option>{routers.map((router) => <option key={router.id} value={router.id}>{router.name} ({router.ip_address})</option>)}</select></Field>
              <div className="grid grid-cols-3 gap-3"><div className="col-span-2"><Field label="Dirección de red"><input required value={form.network_address} onChange={(e) => setForm({ ...form, network_address: e.target.value })} placeholder="192.168.15.0" className="input font-mono" /></Field></div><Field label="CIDR"><select value={form.prefix_length} onChange={(e) => setForm({ ...form, prefix_length: Number(e.target.value) })} className="input">{[30,29,28,27,26,25,24,23,22,21,20,19,18,17,16].map((prefix) => <option key={prefix} value={prefix}>/{prefix}</option>)}</select></Field></div>
              <Field label="Tipo de uso"><select value={form.usage_type} onChange={(e) => setForm({ ...form, usage_type: e.target.value })} className="input"><option value="static">IP estática</option><option value="dhcp">DHCP</option><option value="pppoe_pool">Pool PPPoE</option></select></Field>
