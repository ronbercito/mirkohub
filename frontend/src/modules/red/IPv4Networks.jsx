            ...
            <tbody className="divide-y divide-slate-800/70">
              {loading ? <tr><td colSpan="5" className="p-8 text-center text-slate-500">Cargando redes...</td></tr>
                ...
                : rows.map((network) => <tr key={network.id} className="hover:bg-slate-800/40">
                  ...
                  <td className="p-3 min-w-44"><div className="flex justify-between text-[11px] text-slate-400 mb-1"><span>{network.used_ips} asignadas</span><span>{network.available_ips ?? Math.max((network.usable_hosts || 0) - (network.used_ips || 0), 0)} disponibles</span></div><div className="h-2 rounded-full bg-slate-800 overflow-hidden"><div className="h-full bg-cyan-500" style={{ width: `${Math.min(network.usage_percent, 100)}%` }} /></div></td>
