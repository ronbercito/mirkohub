...
import { CalendarDays, Check, ChevronLeft, ChevronRight, CreditCard, UserRound, Wifi, X } from "lucide-react";
      ...
      <form onSubmit={onSubmit}>
        <div className="p-6 min-h-[400px]">
          ...
          {step === 3 && <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-5"><Card title="Internet y MikroTik" icon={Wifi}><Field label="MikroTik *"><select required value={formData.router_id} onChange={e=>setFormData({...formData,router_id:e.target.value,ipv4_network_id:""})}><option value="" disabled>Selecciona un MikroTik</option>{mikrotiks.map(router=><option key={router.id} value={router.id}>{router.name}</option>)}</select></Field><Field label="Plan de internet *"><select required value={formData.plan_id} onChange={e=>setFormData({...formData,plan_id:e.target.value})}><option value="" disabled>Selecciona un plan</option>{activePlans.map(plan=><option key={plan.id} value={plan.id}>{plan.name} — S/. {Number(plan.price).toFixed(2)}</option>)}</select></Field><Field label="Tipo de conexión"><select value={formData.connection_type} onChange={e=>setFormData({...formData,connection_type:e.target.value,ipv4_network_id:"",ip_address:""})}><option value="PPPoE">PPPoE</option><option value="IP Estática">IP estática</option><option value="DHCP">DHCP</option></select></Field>
              ...
              {formData.connection_type === "PPPoE" && <><Field label="Usuario PPPoE"><input required value={formData.pppoe_user || ""} onChange={e=>setFormData({...formData,pppoe_user:e.target.value})} placeholder="usuario_pppoe" /></Field><Field label="Clave PPPoE"><input required type="password" value={formData.pppoe_password || ""} onChange={e=>setFormData({...formData,pppoe_password:e.target.value})} placeholder="Clave del usuario PPPoE" /></Field><p className="text-[11px] text-slate-400 -mt-2">El plan se aplicará como perfil PPPoE en el MikroTik.</p></>}</Card><Card title="Instalación y tecnología" icon={Wifi}><Field label="Tecnología"><select value={formData.technology || "fiber"} onChange={e=>setFormData({...formData,technology:e.target.value})}><option value="fiber">Fibra óptica</option><option value="wireless">Inalámbrico</option></select></Field><Field label="Zona"><select value={formData.zone_id || ""} onChange={e=>setFormData({...formData,zone_id:e.target.value,nap_box_id:"",nap_box:"",nap_port:""})}><option value="">Sin zona asignada</option>{zones.map(zone=><option key={zone.id} value={zone.id}>{zone.name}</option>)}</select></Field>{(formData.technology || "fiber") === "fiber" ? <><Field label="Caja NAP"><select value={formData.nap_box_id || ""} onChange={e=>setFormData({...formData,nap_box_id:e.target.value,nap_port:"",nap_box:""})}><option value="">{formData.zone_id ? "Sin caja NAP" : "Primero selecciona una zona"}</option>{napBoxesForZone.map(box=><option key={box.id} value={box.id}>{box.name}</option>)}</select></Field><Field label="Puerto NAP"><select required disabled={!nap || availableNapPorts.length === 0} value={formData.nap_port ?? ""} onChange={e=>setFormData({...formData,nap_port:e.target.value === "" ? "" : Number(e.target.value)})}><option value="">{!nap ? "Primero selecciona una caja NAP" : availableNapPorts.length === 0 ? "No hay puertos libres" : "Selecciona un puerto libre"}</option>{availableNapPorts.map(port=><option key={port} value={port}>Puerto {port}</option>)}</select></Field>{nap && availableNapPorts.length === 0 && <p className="text-[11px] text-rose-300 -mt-2">Esta caja NAP no tiene puertos disponibles.</p>}<Field label="Serie ONU"><input value={formData.onu_sn || ""} onChange={e=>setFormData({...formData,onu_sn:e.target.value})} placeholder="SN / MAC de la ONU" /></Field><Field label="Potencia de la ONU (dBm)"><input type="number" step="0.1" value={formData.optical_power_dbm ?? ""} onChange={e=>setFormData({...formData,optical_power_dbm:e.target.value})} placeholder="-19.5" /></Field></> : <><Field label="Conectado a *"><select required value={formData.monitoring_equipment_id || ""} onChange={e=>setFormData({...formData,monitoring_equipment_id:e.target.value})}><option value="">Selecciona un equipo</option>{equipment.map(item=><option key={item.id} value={item.id}>{item.name}{item.ip_address ? ` — ${item.ip_address}` : ""}</option>)}</select></Field><Field label="Tipo de antena"><input value={formData.antenna_type || ""} onChange={e=>setFormData({...formData,antenna_type:e.target.value})} placeholder="Ej. LiteBeam 5AC" /></Field><Field label="IP administración"><input value={formData.management_ip || ""} onChange={e=>setFormData({...formData,management_ip:e.target.value})} placeholder="192.168.x.x" /></Field></>}</Card></div>}
        </div>
