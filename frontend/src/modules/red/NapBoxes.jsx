...
const EMPTY = { name: "", location: "", latitude: "", longitude: "", ports: 8, details: "", zone_id: "" };
    ...
    <div className="bg-slate-900/90 border border-slate-800 rounded-2xl overflow-visible shadow-xl">
      ...
      {rows.length === 0 ? <div className="p-10 text-center text-sm text-slate-500">Aún no hay cajas NAP registradas.</div> :
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 p-4">{rows.map(box => <div key={box.id} className="rounded-xl border border-slate-800 bg-slate-950/50 p-4">
          ...
          <div className="mt-4 flex flex-wrap gap-1.5">{Array.from({ length: box.ports }, (_, i) => i + 1).map(port => { const client = box.assigned_ports[String(port)] || box.assigned_ports[port]; return <button type="button" key={port} onClick={() => setSelectedPort({ boxId: box.id, port, client })} className={`group relative z-10 hover:z-30 focus:z-30 w-8 h-8 rounded-lg text-xs font-bold flex items-center justify-center transition hover:scale-110 focus:outline-none focus:ring-2 focus:ring-cyan-400 ${client ? "bg-rose-500/20 text-rose-300 border border-rose-500/30" : "bg-emerald-500/15 text-emerald-300 border border-emerald-500/25"}`} aria-label={client ? `Puerto ${port} ocupado por ${client.client_name}` : `Puerto ${port} libre`}>{port}{client && <span className="pointer-events-none absolute z-50 bottom-full left-1/2 mb-2 w-52 -translate-x-1/2 rounded-lg border border-rose-500/30 bg-slate-900 px-3 py-2 text-left text-[11px] font-normal text-slate-100 opacity-0 shadow-xl transition-opacity group-hover:opacity-100 group-focus:opacity-100"><b className="text-rose-300">Puerto {port} ocupado</b><br />{client.client_name}</span>}</button>; })}</div>
          {selectedPort?.boxId === box.id && <div className={`mt-3 rounded-lg border px-3 py-2 text-xs ${selectedPort.client ? "border-rose-500/25 bg-rose-500/10 text-rose-200" : "border-emerald-500/25 bg-emerald-500/10 text-emerald-200"}`}>{selectedPort.client ? <>Puerto {selectedPort.port}: asignado a <b>{selectedPort.client.client_name}</b></> : <>Puerto {selectedPort.port}: libre para asignar.</>}</div>}
